...
use core::types::{Cell, FkPolicy, Row, RowId, TableId, Value, Version};
use index::IndexManager;
use query::QueryResult;
use replication::ReplicaState;
use sqlparser::ast::{
    self, Expr, FromTable, ObjectName, SelectItem, SetExpr, Statement, TableFactor, TableObject,
};
...
impl SqlExecutor {
    ...
    fn execute_stmt(
        ...
        match stmt {
            ...
            Statement::Update { table, assignments, selection, .. } => {
                self.exec_update(replica, indexes, table, assignments, selection.as_ref(), params)
            }
            ...
            _ => Err(CrdtError::ParseError(format!("unsupported statement type"))),
        }
    }
    ...
    fn exec_insert(
        ...
        let table_name = match &insert.table {
            ...
            _ => return Err(CrdtError::ParseError("INSERT: unsupported table object".to_string())),
        };
        ...
        let rows_to_insert = match source.body.as_ref() {
            ...
            _ => return Err(CrdtError::ParseError("only VALUES supported in INSERT".to_string())),
        };
        ...
        for row_vals in rows_to_insert {
            ...
            let pk_val = cells.get(&pk_col.name)
            ...
            for (col_name, val) in &cells {
                row.cells.insert(col_name.clone(), Cell::new(val.clone(), version.clone()));
            }
                ...
                if let Some(val) = cells.get(&col.name) {
                    if *val != Value::Null {
                        replica.uniqueness.claim(
                            &table_name, &col.name, &val.to_string(), &row_id, version.clone(),
                        );
                    }
                }
            }
    ...
    fn exec_update(
        ...
        let table_name = match &table.relation {
            ...
            _ => return Err(CrdtError::ParseError("UPDATE: simple table only".to_string())),
        };
        ...
        let matching_ids: Vec<RowId> = replica
            ...
            .filter(|row| selection.map_or(true, |sel| eval_predicate(sel, row, params).unwrap_or(false)))
            .map(|r| r.id.clone())
            .collect();
            ...
            for assign in assignments {
                ...
                if let Some(col) = schema.column(&col_name) {
                    if col.unique && new_val != Value::Null {
                        replica.uniqueness.claim(
                            &table_name, &col_name, &new_val.to_string(), &row_id, version.clone(),
                        );
                    }
                }

                updated.cells.insert(col_name, Cell::new(new_val, version.clone()));
            }
        ...
        let matching_ids: Vec<RowId> = replica
            ...
            .filter(|row| {
                delete.selection.as_ref()
                    .map_or(true, |sel| eval_predicate(sel, row, params).unwrap_or(false))
            })
            .map(|r| r.id.clone())
            .collect();
    ...
    fn exec_select(
        ...
        let table_name = match &select.from[0].relation {
            TableFactor::Table { name, .. } => object_name_to_string(name),
            _ => return Err(CrdtError::ParseError("FROM must be a simple table".to_string())),
        };

        let schema = replica.schemas.get(&table_name)
            .ok_or_else(|| CrdtError::TableNotFound(table_name.clone()))?;

        let all_cols: Vec<String> = schema.columns.iter().map(|c| c.name.clone()).collect();

        let output_cols: Vec<String> = if select.projection.iter().any(|p| matches!(p, SelectItem::Wildcard(_) | SelectItem::QualifiedWildcard(_, _))) {
            all_cols.clone()
        } else {
            select.projection.iter().map(|p| match p {
        ...
        if let Some(order_by) = &query.order_by {
            rows.sort_by(|a, b| {
                for order_expr in &order_by.exprs {
                    ...
                    let cmp = va.cmp(vb);
                    let cmp = if order_expr.asc == Some(false) { cmp.reverse() } else { cmp };
        ...
        FromTable::WithFromKeyword(tables) | FromTable::WithoutKeyword(tables) => {
            match tables.first() {
                Some(twj) => match &twj.relation {
                    ...
                    _ => Err(CrdtError::ParseError("DELETE: complex FROM not supported".to_string())),
                },
                None => {
                    // Fall back to tables list
                    delete.tables.first()
                        .map(|n| object_name_to_string(n))
                        .ok_or_else(|| CrdtError::ParseError("DELETE: no table specified".to_string()))
                }
            }
        }
    }
...
fn eval_col_ref(expr: &Expr, row: &Row) -> CrdtResult<Value> {
    match expr {
        Expr::Identifier(id) => Ok(row.cells.get(&id.value).map(|c| c.value.clone()).unwrap_or(Value::Null)),
        Expr::CompoundIdentifier(parts) => {
            let col = parts.last().map(|p| p.value.as_str()).unwrap_or("");
            Ok(row.cells.get(col).map(|c| c.value.clone()).unwrap_or(Value::Null))
        }
        _ => Err(CrdtError::ParseError(format!("cannot evaluate expression as column ref"))),
    }
}
...
mod tests {
    ...
    fn setup_replica(peer_id: &str) -> (ReplicaState, IndexManager) {
        ...
        let users_schema = TableSchema {
            ...
            columns: vec![
                ColumnSchema { name: "id".into(), data_type: DataType::Text, nullable: false, unique: false, primary_key: true, default_value: None },
                ColumnSchema { name: "email".into(), data_type: DataType::Text, nullable: false, unique: true, primary_key: false, default_value: None },
                ColumnSchema { name: "name".into(), data_type: DataType::Text, nullable: true, unique: false, primary_key: false, default_value: None },
            ],
            foreign_keys: vec![],
            indexes: vec![],
        };
        ...
        let orders_schema = TableSchema {
            ...
            columns: vec![
                ColumnSchema { name: "id".into(), data_type: DataType::Text, nullable: false, unique: false, primary_key: true, default_value: None },
                ColumnSchema { name: "user_id".into(), data_type: DataType::Text, nullable: false, unique: false, primary_key: false, default_value: None },
                ColumnSchema { name: "status".into(), data_type: DataType::Text, nullable: false, unique: false, primary_key: false, default_value: None },
                ColumnSchema { name: "total_cents".into(), data_type: DataType::Integer, nullable: false, unique: false, primary_key: false, default_value: Some(Value::Integer(0)) },
            ],
            foreign_keys: vec![ForeignKeyDef { column: "user_id".into(), ref_table: "users".into(), ref_column: "id".into(), on_delete: FkPolicy::Tombstone }],
            indexes: vec![],
        };
    ...
    fn exec(replica: &mut ReplicaState, indexes: &mut IndexManager, sql: &str) {
        SqlExecutor::new().execute(replica, indexes, sql, &[]).unwrap_or_else(|e| panic!("{sql}: {e}"));
    }
    ...
    fn insert_and_select_star() {
        ...
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u1', 'alice@x.com', 'Alice')");
    ...
    fn update_one_column() {
        let (mut r, mut idx) = setup_replica("A");
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u1', 'alice@x.com', 'Alice')");
        exec(&mut r, &mut idx, "UPDATE users SET name = 'Alice Cooper' WHERE id = 'u1'");
    ...
    fn where_filter_works() {
        let (mut r, mut idx) = setup_replica("A");
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u1', 'a@x.com', 'Alice')");
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u2', 'b@x.com', 'Bob')");
    ...
    fn limit() {
        ...
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u1', 'a@x.com', 'Alice')");
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u2', 'b@x.com', 'Bob')");
        exec(&mut r, &mut idx, "INSERT INTO users (id, email, name) VALUES ('u3', 'c@x.com', 'Charlie')");
